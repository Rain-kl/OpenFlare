...
- 修复 WAF IP 组保存后在线节点仍按旧成员拦截的问题。保存手动、自动或订阅 IP 组后会立即通过现有 `waf_ip_groups` WebSocket 消息 best-effort 广播给 Agent，Agent 在完整配置 no-op 路径也会重新校准引用 IP 组，确保 `waf_ip_groups.json` 不再残留被移除的 IP/CIDR。
