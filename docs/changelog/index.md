...
- Agent 二进制替换升级时，若本地观测补传缓冲仍是旧格式或损坏，则直接删除该文件并在运行中重建，避免半迁移数据或阻塞心跳。
- Zone 流量图：顶部独立访客为整窗去重；曲线标明为分桶 UV（桶内去重，不可跨桶相加）。
- 明确 OpenResty 健康权威：当前 status/message 以节点表为准，ClickHouse 仅存 status 与连接时序（不含 message）。
- 看板与节点观测不再展示或采集宿主机网卡入/出站；网络趋势仅保留访问日志的已提供/接收数据。
- 宿主机磁盘读/写趋势改为按小时速率（B/s）展示，摘要为近 24 小时平均速率。
- 修复节点地图在缺少精确经纬度时把香港/新加坡/台湾等地区回退到错误占位坐标的问题；补全质心数据并改进复合地名匹配。
...
- 优化 Pages 部署包校验性能：上传时不再为包内每个文件计算哈希，文件清单仅记录路径与声明大小；完整性改由整包校验和保障，并避免将整个压缩包装入内存后再逐文件读取。
