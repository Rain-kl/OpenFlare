...
describe('WAF route binding order', () => {
  beforeEach(() => {
    ...
    vi.mocked(WafService.updateSiteRuleGroups).mockImplementation(async (_routeID, ids) => ({
      route_id: 9,
      global_rule_group: rule(99, '全局规则', true),
      rule_groups: [rule(1, '规则 A'), rule(2, '规则 B'), rule(3, '规则 C')],
  ...
  it('keeps the global rule fixed and submits custom rules in UI order', async () => {
    ...
    expect(screen.queryByRole('button', {name: '上移全局规则'})).not.toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(WafService.updateSiteRuleGroups).toHaveBeenCalledWith(9, [3, 1, 2]);
    });
  });
});
